...
<template>
    <v-container v-if="auth.token && auth.userInfo">
        <UserInfo/>
        <v-sheet elevation="2" rounded="lg" :class="xs ?'pa-1' : 'pa-2'" align-tabs="start">
            <v-tabs v-model="tab" class="bg-surface" color="surface" grow>
                ...
                <v-spacer/>
                <v-btn v-if="!xs" @click="showNotificationsTable=true" icon="mdi-bell" :color="unreadNotifications > 0 ? 'error' : 'secondary' " class="align-self-center notification-button ms-2"/>
                <v-btn v-if="!xs" @click="showCouponTable=true" icon="mdi-cash" color="secondary" class="align-self-center notification-button ms-2"/>
            </v-tabs>
        </v-sheet>
