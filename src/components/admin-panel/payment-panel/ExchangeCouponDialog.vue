            ...
            <v-card class="pa-4">
                ...
                <v-col cols="12" class="text-center">
                    <div v-if="isMobile">
                        ...
                        <div v-show="scanning" id="qr-reader" class="overflow-hidden mt-4 mx-auto rounded-lg elevation-2" style="max-width: 320px;"></div>
                    </div>
                    <v-alert 
                        v-else 
                        type="warning" 
                ...
                <v-col cols="12" v-if="scannedData">
                    <v-card variant="tonal" color="success" class="text-center pa-3">
                        ...
                        <div class="text-caption">ID Cupó: {{ scannedData.couponId }} | ID Stock: {{ scannedData.stockId }} | Quantitat: {{ scannedData.amount }}</div>
                    </v-card>
                </v-col>
...
const processQrCode = async (text) => {
    try {
        ...
        if (!text.startsWith("app://exchange")) {
            throw new Error("El codi QR escanejat no és un tiquet vàlid d'aquesta aplicació.");
        }
