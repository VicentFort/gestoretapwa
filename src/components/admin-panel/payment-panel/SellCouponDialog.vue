<template>
    <v-container>
        <v-form @submit.prevent="submitForm" ref="form" v-model="valid">
            ...
            <v-col>
                <v-row rows="12" md="6">
                    <v-select v-if="auth.fallaAdminInfo.coupons" :items="coupons" :item-props="couponProps" v-model="coupon" label="Tiquet a vendre"/>
                </v-row>
            </v-col>
            <v-col>
                <v-row rows="12" md="6">
                    <v-number-input label="Quantitat de tiquets a vendre" v-model="couponAmount" :min="1" :step="1" :max="getMaxStock"/>
                </v-row>
            </v-col>
            <v-col>
                <v-row rows="12" md="6">
