    ...
    <v-form @submit.prevent="submitForm" ref="form" v-model="valid">
      <v-card>
        ...
        <v-col v-if="selectedType && selectedType.id == 3">
          <v-row rows="12" md="6">
            <v-select
              v-if="auth.fallaAdminInfo.users"
              :items="users"
              :item-props="userProps"
              v-model="feeUser"
              label="Usuari a cobrar quota"
            />
          </v-row>
        </v-col>
        <v-col v-if="selectedType && selectedType.id == 3">
