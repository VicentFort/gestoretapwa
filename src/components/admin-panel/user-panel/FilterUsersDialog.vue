...
const submitForm = async () => {
  try {
    let filtered = auth.fallaAdminInfo.users.filter((user) => {
      const meetsAccessType =
        aType.value != null ? user.accessType === aType.value : true;
      const meetsDate =
        date.value != null
          ? new Date(user.joinDate) <= new Date(date.value)
          : true;
      const meetsBDay =
        birthday.value != null
          ? new Date(user.birthday) <= new Date(birthday.value)
          : true;
      const meetsNickname =
        nickname.value != "" ? user.nickname.includes(nickname.value) : true;
      const meetsName =
        name.value != ""
          ? (user.name + " " + user.surname).includes(name.value)
          : true;
      return (
        meetsAccessType && meetsDate && meetsBDay && meetsNickname && meetsName
      );
    });
