  ...
  <v-container>
    <v-form @subimt.prevent="handleLogin" ref="form" v-model="valid">
      <v-card>
        ...
        <v-divider />
        <v-card-text class=""> Item: {{ loan.itemName }} </v-card-text>
        <v-card-text class=""> Contacte: {{ loan.contactName }} </v-card-text>
        <v-card-text class=""> Quantitat: {{ loan.amount }} </v-card-text>
        <v-divider />
        <v-row>
          <v-col cols="12" md="6">
            <v-select
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            @click="submitForm"
            :disabled="selectedStore == null || message == null"
          />
