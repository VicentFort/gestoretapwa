...
const submitForm = async () => {
  try {
    let filtered = auth.fallaAdminInfo.inventoryItems.filter((item) => {
      const meetsName =
        name.value != "" ? item.name.includes(name.value) : true;
      const meetsCategory =
        category.value != "" ? item.category.includes(category.value) : true;
      const meetsStore = store.value
        ? item.stocks.some((stock) => stock.storeId === store.value.id)
        : true;
